...
@Component({
  selector: 'app-reg-prin',
  templateUrl: './reg-prin.component.html',
  styleUrls: ['./reg-prin.component.css']
})
export class RegPrinComponent {

  prin: Prin=new Prin(); //teacher object
  ...
  prinRegisterform=this.builder.group({ //form group for teacher reg form
  ...
  sendPriData() { //invoke this method when click submit teacher reg data
    if (this.prinRegisterform.valid) {
      this.UserRegService.sendPriData(this.prin).subscribe(response => { //cordinate backend and frontend data
        console.log('Data sent successfully!');
        console.log(this.prinRegisterform.value);
